...
int Amesos_Klu::PerformNumericFactorization( ) 
{
  ...
  if (MyPID_ == 0) {
    ...
    if ( factor_with_pivoting ) {
      ...
      PrivateKluData_->Numeric_ = rcpWithDealloc( amesos_klu_factor(&Ap[0], Ai, Aval, &*PrivateKluData_->Symbolic_, &*PrivateKluData_->common_),    
                                                  deallocFunctorDeleteWithCommon<klu_numeric>(PrivateKluData_->common_,amesos_klu_free_numeric), true );

      numeric_ok =  PrivateKluData_->Numeric_.get()!=NULL && PrivateKluData_->common_->status == KLU_OK ;
      /* EDIT KIM */
      KLU_numeric * Numeric = &*PrivateKluData_->Numeric_;
      double * udiag = reinterpret_cast<double *>(Numeric->Udiag);
      for(int i = 0; i < 16; ++i) 
        printf("\t\tUdiag[%d]: %f\n", i, udiag[i]);
        //std::cout << "Udiag[" << i << "]: " << udiag[i] << "\n";
...
int Amesos_Klu::Solve() 
{
  ...
  if ( MyPID_ == 0) {
    ...
    if (UseTranspose()) {
      ...
      /* EDIT KIM */
      std::cout << "PrivateKluData_->Symbolic_: " << PrivateKluData_->Symbolic_  << "\n";
      std::cout << "PrivateKluData_->Numeric_: " << PrivateKluData_->Numeric_ << "\n";
      std::cout << "PrivateKluData_->common_: " << PrivateKluData_->common_ << "\n";
      std::cout << "SerialXlda_: " << SerialXlda_ << "\n";
      std::cout << "NumVectors_: " << NumVectors_ << "\n";
      /* END KIM */
      ...
      /* EDIT KIM */
      for(int i = 0; i < 16; ++i) {
          std::cout << "XBval: " << SerialXBvalues_[i] << "\n";
      }
      std::cout << "PrivateKluData_->common_: " << PrivateKluData_->common_ << "\n";
      /* END KIM */
    }
