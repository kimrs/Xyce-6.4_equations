...
// ---------------------
//
// Revision Number: $Revision: 1.251 $
//
// Revision Date  : $Date: 2015/09/09 19:26:09 $
//
...
void Instance::setupNoiseSources (Xyce::Analysis::NoiseData & noiseData) 
{
  ...
  noiseData.noiseNames[0] = "onoise_" + getName().getEncodedName();
...
bool Master::loadDAEMatrices(Linear::Matrix & dFdx, Linear::Matrix & dQdx)
{
  ...
  /* EDIT KIM */
  /* Old dFdx is set in the beginning of the for loop */
  ...
  ri.print_strings(tmp_dFdx, ri.dFdx_Strings, ri.getName().getEncodedName());
  /* NOTE KIM */
