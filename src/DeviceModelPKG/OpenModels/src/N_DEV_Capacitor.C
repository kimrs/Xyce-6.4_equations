...
// ---------------------
//
// Revision Number: $Revision: 1.282 $
//
// Revision Date  : $Date: 2015/09/09 19:26:08 $
//
...
void Instance::loadNodeSymbols(Util::SymbolTable &symbol_table) const
{
  ...
  if (loadLeadCurrent)
  {
    //addStoreNode(symbol_table, li_store_dev_i, getName(), "DEV_I");
...
bool Instance::loadDAEQVector ()
{
  ...
  if( loadLeadCurrent )
  {
    ...
    storeLeadQ[li_store_dev_i] = q0;

    double * leadQ = extData.nextLeadCurrQCompRawPtr;
    leadQ[li_branch_data] = q0;
  }
...
bool Master::loadDAEMatrices (Linear::Matrix & dFdx, Linear::Matrix & dQdx)
{
  ...
  for (InstanceVector::const_iterator it = getInstanceBegin(); it != getInstanceEnd(); ++it)
  {
    /* EDIT KIM Clone dQdx in order to see the changes later */
    Epetra_CrsMatrix old(dQdx.epetraObj());
    Linear::Matrix old_dQdx(&old, false); /* SIGABRT if set to true you fucker */
    /* NOTE KIM */
  ...
  Epetra_CrsMatrix tmp(dQdx.epetraObj());
  Linear::Matrix tmp_dQdx(&tmp, false);

  /* tmp_dQdx = dQdx - old_dQdx */
  tmp_dQdx.linearCombo(1.0, tmp_dQdx, -1.0, old_dQdx);
  ci.print_strings(tmp_dQdx, ci.dQdx_Strings, ci.getName().getEncodedName());
