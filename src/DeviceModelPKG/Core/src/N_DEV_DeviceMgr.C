...
// ---------------------
//
// Revision Number: $Revision: 1.708 $
//
// Revision Date  : $Date: 2015/10/14 18:48:56 $
//
...
/* EDIT KIM */
EquationStrings::EquationStrings(Epetra_CrsMatrix * mat):initialized(false) {
    init_streams(mat);
};
 ...
 * Not tested yet
 */
void EquationStrings::clear() {
    for(int i = 0; i < n_stringStreams_; ++i) {
        stringStreams_[i].str("");
    }
};

void EquationStrings::init_streams(Epetra_CrsMatrix * mat) {
    n_stringStreams_ = mat->NumMyNonzeros();
    std::cout << "NUMBER OF NONZEROS IS:  " << n_stringStreams_ << "\n";
    stringStreams_ = new std::stringstream[n_stringStreams_];
    n_rows = mat->NumMyRows();

    row_indices_ = new int[n_rows + 1];
    row_indices_[0] = 0;
    col_indices_ = new int[n_stringStreams_];
    row_lengths_ = new int[n_rows];

    int n_entries_in_col_indices_ = 0;
    for(int i = 0; i < n_rows; ++i) {
        int n_entries;
        double * values;
        int * indices;
        mat->ExtractMyRowView (i, n_entries, values, indices); 
        for(int j = 0; j < n_entries; ++j) 
            col_indices_[n_entries_in_col_indices_  + j] = indices[j];
        
        n_entries_in_col_indices_ += n_entries;
        row_indices_[i+1] = n_entries_in_col_indices_; /* row indices is supposed to point to 
                                                        * the entry in col_indices that row[i]
                                                        * starts
                                                        */
        row_lengths_[i] = n_entries;
    }
    initialized = true;
};

void EquationStrings::init_streams(Epetra_MultiVector * vec, std::string tag_= "") {
    tag = tag_;
    Epetra_CrsMatrix crsmat = ExternData::multivec2crsMat(*vec);
    /* set in number of non zeros. if I don't do that, 
     * 0 streams will be initialized
     */
    init_streams(&crsmat);
};

std::stringstream * EquationStrings::getStringStream(int m, int n) {
//    std::cout << "TAG: " << tag << "\n"; 
    if(!initialized) {
        std::cout << "streams not initialized. Return NULL\n";
        return NULL;
    }

    for(int j = 0; j < row_lengths_[m]; ++j) {
        if(col_indices_[ row_indices_[m] + j] == n)
            return &stringStreams_[ row_indices_[m] + j ];
    }
    std::cout << "could not find stream. Returning NULL\n";
    return NULL;
};

void EquationStrings::print(std::ostream &os) {
    os << "number of streams: ";
    os << n_stringStreams_; 
    os << "\n";
    for(int i = 0; i < n_rows; ++i) {
        for(int j = 0; j < row_lengths_[i]; ++j) {
            os << i;
            os << "\t";
            os << col_indices_[ row_indices_[i] + j];
            os << "\t";
            os << stringStreams_[ row_indices_[i] + j ].str();
            os << "\n";
        }
    }
};
...
DeviceMgr::DeviceMgr(
  ...
  : commandLine_(command_line),
    ...
    dFdx_Strings(), /* EDIT KIM */
    dQdx_Strings(), /* EDIT KIM */
    F_Strings(), /* EDIT KIM */
    Q_Strings(), /* EDIT KIM */
    B_Strings() /* EDIT KIM */
{
...
bool DeviceMgr::initializeAll(Linear::System &linear_system)
{
  ...
  externData_.currLeadCurrFCompPtr = 0;
  externData_.currLeadCurrQCompPtr = 0;
  externData_.currJunctionVCompPtr = 0;
  externData_.nextLeadCurrFCompPtr = 0;
  externData_.nextLeadCurrQCompPtr = 0;
  externData_.nextJunctionVCompPtr = 0;
  externData_.lastLeadCurrFCompPtr = 0;
  externData_.lastLeadCurrQCompPtr = 0;
  externData_.lastJunctionVCompPtr = 0;

  if (DEBUG_DEVICE)
  {
    // get f vector pointer:
    externData_.fVectorPtr  = linear_system.getFVector();
  ...
  if (DEBUG_VOLTLIM)
  {
    ...
    externData_.JTestMatrixPtr = linear_system.getJacTestMatrix();
    bsuccess = bsuccess && (externData_.JTestMatrixPtr != 0);

    // get test matrix:
    externData_.FTestMatrixPtr = linear_system.getdFdxTestMatrix();
    bsuccess = bsuccess && (externData_.FTestMatrixPtr != 0);
    externData_.QTestMatrixPtr = linear_system.getdQdxTestMatrix();
    bsuccess = bsuccess && (externData_.QTestMatrixPtr != 0);

    // get dxVoltlim vector pointer:
    externData_.dxVoltlimVectorPtr = linear_system.getDxVoltlimVector() ;
    bsuccess = bsuccess && (externData_.dxVoltlimVectorPtr != 0);

    // create Jdx2 vector pointer: (old DAE)
    externData_.Jdx2VectorPtr = linear_system.getJDX2Vector();
    bsuccess = bsuccess && (externData_.Jdx2VectorPtr != 0);

    // create Jdx2 vector pointer:
    externData_.Fdx2VectorPtr = linear_system.getFDX2Vector();
    bsuccess = bsuccess && (externData_.Fdx2VectorPtr != 0);
    externData_.Qdx2VectorPtr = linear_system.getQDX2Vector();
  ...
  {
    numJacStaVectorPtr_ = linear_system.builder().createStateVector();
    numJacSolVectorPtr_ = linear_system.builder().createVector();
    numJacStoVectorPtr_ = linear_system.builder().createStoreVector();

    externData_.numJacRHSVectorPtr = linear_system.builder().createVector();
    externData_.numJacFVectorPtr   = linear_system.builder().createVector();
    externData_.numJacQVectorPtr   = linear_system.builder().createVector();
    externData_.perturbVectorPtr   = linear_system.builder().createVector();
    externData_.numJacLoadFlagPtr  = linear_system.builder().createVector();
  }
  ...
  diagonalVectorPtr_  = linear_system.builder().createVector();
  ...
  const InstanceBlock &         instance_block)
{
  ...
  // Using setupIOName removes the "yxxx" or "uxxx" part of the name.  However that leaves
  // an ambiguous device name to resolve after that (i.e. ymemristor mr1 becomes ymemristor!mr1 and
  // the value returned by setupIOName is "mr1" which looks like a mosfet device called "r1".  Thus
  // we need to use the full instanste name here an not the IO name.
  //
  ...
  isLinearSystem_ = isLinearSystem_ && device.isLinearDevice();
...
void DeviceMgr::debugOutput1()
{
  ...
  if (isActive(Diag::DEVICE_DUMP_VECTORS) && solState_.debugTimeFlag)
  {
    ...
    // sprintf(fn_fv, "fvector.%03d.txt", newton_iteration_count);
    {
      std::ostringstream oss;
      oss << "fvector." << std::setw(3) << std::setfill('0') << newton_iteration_count << ".txt";

      // Note: this needs to change sign to match Spice.
      (externData_.fVectorPtr)->scale(-1.0);
      (externData_.fVectorPtr)->writeToFile(oss.str().c_str());
      (externData_.fVectorPtr)->scale(-1.0);
    }
    ...
    if (DEBUG_VOLTLIM)
    {
      ...
      // sprintf(fn_dxvl, "dxVL.%03d.txt", newton_iteration_count);
      {
        std::ostringstream oss;
        oss << "dxVL." << std::setw(3) << std::setfill('0') << newton_iteration_count << ".txt";
        (externData_.dxVoltlimVectorPtr)->writeToFile(oss.str().c_str());
      }

      // jdx2-vector
      // char fn_jdx2[256]; for (int ich = 0; ich < 256; ++ich) fn_jdx2[ich] = 0;
      // sprintf(fn_jdx2, "Jdx2.%03d.txt", newton_iteration_count);
      {
        std::ostringstream oss;
        oss << "Jdx2." << std::setw(3) << std::setfill('0') << newton_iteration_count << ".txt";
        (externData_.Jdx2VectorPtr)->writeToFile(oss.str().c_str());
      }
...
void DeviceMgr::debugOutput2()
{
  ...
  if (isActive(Diag::DEVICE_DUMP_VECTORS) && solState_.debugTimeFlag)
  {
    ...
    if (DEBUG_VOLTLIM)
    {
      // the voltlim dx vector.
      // char fn_dxvl[256]; for (int ich = 0; ich < 256; ++ich) fn_dxvl[ich] = 0;
      // sprintf(fn_dxvl, "dxVL.%03d.%03d.txt", outputStepNumber, newton_iteration_count);
      {
        std::ostringstream oss;
        oss << "dxVL." << std::setw(3) << std::setfill('0') << outputStepNumber 
          << "." << std::setw(3) << std::setfill('0') << newton_iteration_count << ".txt";
        (externData_.dxVoltlimVectorPtr)->writeToFile(oss.str().c_str());
      }

      // Fdx2-vector
      // char fn_fdx2[256]; for (int ich = 0; ich < 256; ++ich) fn_fdx2[ich] = 0;
      // sprintf(fn_fdx2, "Fdx2.%03d.%03d.txt", outputStepNumber, newton_iteration_count);
      {
        std::ostringstream oss;
        oss << "Fdx2." << std::setw(3) << std::setfill('0') << outputStepNumber 
          << "." << std::setw(3) << std::setfill('0') << newton_iteration_count << ".txt";
        (externData_.Fdx2VectorPtr)->writeToFile(oss.str().c_str());
      }

      // Qdx2-vector
      // char fn_qdx2[256]; for (int ich = 0; ich < 256; ++ich) fn_qdx2[ich] = 0;
      // sprintf(fn_qdx2, "Qdx2.%03d.%03d.txt", outputStepNumber, newton_iteration_count);
      {
        std::ostringstream oss;
        oss << "Qdx2." << std::setw(3) << std::setfill('0') << outputStepNumber 
          << "." << std::setw(3) << std::setfill('0') << newton_iteration_count << ".txt";
        (externData_.Qdx2VectorPtr)->writeToFile(oss.str().c_str());
      }
    }
  }
}
  ...
  Linear::Vector * lastStoVectorPtr)
{
  ...
  if (solState_.twoLevelNewtonCouplingMode == Nonlinear::INNER_PROBLEM)
  {
    ...
    for (DeviceVector::iterator iter = pdeDevicePtrVec_.begin(), end = pdeDevicePtrVec_.end(); iter != end; ++iter)
    {
      tmpBool = (*iter)->updateState (externData_.nextSolVectorRawPtr, externData_.nextStaVectorRawPtr, externData_.nextStoVectorRawPtr);
      bsuccess = bsuccess && tmpBool;
    }
  }
  else
  {
    for (DeviceVector::iterator iter = devicePtrVec_.begin(), end = devicePtrVec_.end(); iter != end; ++iter)
    {
      bsuccess = (*iter)->updateState(externData_.nextSolVectorRawPtr, externData_.nextStaVectorRawPtr, externData_.nextStoVectorRawPtr);
    }
  }
  ...
  Linear::Matrix * tmpdFdxMatrixPtr)
{
  ...
  {
    dFdx_Strings.init_streams(&externData_.dFdxMatrixPtr->epetraObj()); /* EDIT KIM */
    dQdx_Strings.init_streams(&externData_.dQdxMatrixPtr->epetraObj());
    //F_Strings.init_streams(&externData_.daeFVectorPtr->epetraObj());
    //Q_Strings.init_streams(&externData_.daeQVectorPtr->epetraObj());
    Epetra_MultiVector mvec = externData_.daeFVectorPtr->epetraObj();
    Epetra_CrsMatrix mat = ExternData::multivec2crsMat(mvec);
    ...
    {
      bsuccess = (*it)->loadDAEMatrices(*externData_.dFdxMatrixPtr , *externData_.dQdxMatrixPtr);
    }
    ...
    if(first) {
        ...
        os.open("dFdx_Strings.csv");
        dFdx_Strings.print(os);
        os.close();

        os.open("dQdx_Strings.csv");
        dQdx_Strings.print(os);
        os.close(); 
    
        first = false;
    }
    os.open("dFdx.csv");
    externData_.dFdxMatrixPtr->printPetraObject(os);
    os.close();

    os.open("dQdx.csv");
    externData_.dQdxMatrixPtr->printPetraObject(os);
    os.close();
    /* DONE KIM */
  }
...
bool DeviceMgr::loadDAEVectors(
  ...
  Linear::Vector * tmpCurLeadFCompVectorPtr,
  Linear::Vector * tmpLastLeadFCompVectorPtr,
  Linear::Vector * tmpLeadQCompVectorPtr,
  Linear::Vector * tmpJunctionVCompVectorPtr,
  Linear::Vector * tmpCurrJunctionVCompVectorPtr,
  Linear::Vector * tmpLastJunctionCompVectorPtr,
  ...
  Linear::Vector * tmpdQdxdVpVectorPtr)
{
  ...
  externData_.currLeadCurrFCompPtr = tmpCurLeadFCompVectorPtr;
  externData_.lastLeadCurrFCompPtr= tmpLastLeadFCompVectorPtr;
  externData_.nextLeadCurrQCompPtr = tmpLeadQCompVectorPtr;
  externData_.nextJunctionVCompPtr = tmpJunctionVCompVectorPtr;
  externData_.currJunctionVCompPtr = tmpCurrJunctionVCompVectorPtr;
  externData_.lastJunctionVCompPtr = tmpLastJunctionCompVectorPtr;
  ...
  else
  {
    ...
    for(int i=0; i< numDevices; ++i)
    {
      /* EDIT KIM */ devicePtrVec_.at(i)->F_strings = &F_Strings;      
      /* EDIT KIM */ devicePtrVec_.at(i)->Q_strings = &Q_Strings;      
      /* EDIT KIM */ devicePtrVec_.at(i)->B_strings = &B_Strings;      
      /* EDIT KIM */ devicePtrVec_.at(i)->externData = externData_;
  ...
  /*EDIT PRINT KIM */ 
  std::ofstream ofs;
  ofs.open("F_Strings.txt");
  F_Strings.print(ofs);
  ofs.close();
  F_Strings.clear();

  ofs.open("dae_F.txt");
  externData_.daeFVectorPtr->printPetraObject(ofs);
  ofs.close();

  ofs.open("Q_Strings.txt");
  Q_Strings.print(ofs);
  ofs.close();
  Q_Strings.clear();

  ofs.open("dae_Q.txt");
  externData_.daeQVectorPtr->printPetraObject(ofs);
  ofs.close();

  ofs.open("B_Strings.txt");
  B_Strings.print(ofs);
  ofs.close();
  B_Strings.clear();

  ofs.open("dae_B.txt");
  externData_.daeBVectorPtr->printPetraObject(ofs);
  ofs.close();
...
// copy over the passed pointers:
  ...
  for ( ; it != end; ++it)
  {
    (*it)->loadBVectorsforAC(externData_.bVecRealRawPtr, externData_.bVecImagRawPtr);
  }
...
{
#if 0
  if (noiseDataVec.empty())
  {
    noiseDataVec.resize(instancePtrVec_.size());
  }
  else
  {
    if (noiseDataVec.size() != instancePtrVec_.size())
    {
      Report::DevelFatal message;
      message.in("DeviceMgr::deRegisterFastSources");
      message << "Size of noise vector and device vector don't match.";
      message << "noiseDataVec.size = " << noiseDataVec.size()
...
// Creation Date :
//-----------------------------------------------------------------------------
bool DeviceMgr::getBMatrixEntriesforMOR(
...
bool DeviceMgr::setupRawVectorPointers_ ()
{
  ...
  if (externData_.currLeadCurrFCompPtr != 0)
  {
    externData_.currLeadCurrFCompRawPtr = &((*externData_.currLeadCurrFCompPtr)[0]);
  }

  if (externData_.lastLeadCurrFCompPtr != 0)
  {
    externData_.lastLeadCurrFCompRawPtr = &((*externData_.lastLeadCurrFCompPtr)[0]);
  }
  ...
  if (externData_.currLeadCurrQCompPtr != 0)
  {
    externData_.currLeadCurrQCompRawPtr = &((*externData_.currLeadCurrQCompPtr)[0]);
  }

  if (externData_.lastLeadCurrQCompPtr != 0)
  {
    externData_.lastLeadCurrQCompRawPtr = &((*externData_.lastLeadCurrQCompPtr)[0]);
  }
  ...
  if (externData_.currJunctionVCompPtr != 0)
  {
    externData_.currJunctionVCompRawPtr = &((*externData_.currJunctionVCompPtr)[0]);
  }

  if (externData_.lastJunctionVCompPtr != 0)
  {
    externData_.lastJunctionVCompRawPtr = &((*externData_.lastJunctionVCompPtr)[0]);
  }
...
bool DeviceMgr::setupRawMatrixPointers_()
{
    for (InstanceVector::iterator it = instancePtrVec_.begin(), end = instancePtrVec_.end(); it != end; ++it) {
        DeviceInstance * dev = (*it);
        dev->dFdx_Strings = &dFdx_Strings;
        dev->dQdx_Strings = &dQdx_Strings;
        dev->F_Strings = &F_Strings;
        dev->Q_Strings = &Q_Strings;
        dev->B_Strings = &B_Strings;
  ...
  char * buf, int bsize, int & pos, N_PDS_Comm * comm, bool pack) const
{
  ...
  if (pack)
  {
    size_t size=solState_.ltraTimePoints_.size();
  ...
  double &              value)
{
  ...
  // bool found = device_manager.getParamNoReduce(name, value);

  // int found_count = found ? 1 : 0;
  // Parallel::AllReduce(comm, MPI_SUM, &found_count, 1);

  // if (found_count)
  // {
  //   Parallel::AllReduce(comm, MPI_SUM, &value, 1);
  //   value /= found_count;
  // }

  // double value2 = op ? (*op)(comm, Util::Op::OpData()).real() : 0.0;
  // // if (found_count)
  // //   value2 /= found_count;

  // if (value != value2)
  //   Report::DevelFatal() << "op value " << name << " different, Op = " << value2 << ", getParam = " << value << ", Found Count = " << found_count;
...
// //-----------------------------------------------------------------------------
// // Function      : DeviceMgr::getParameter
// // Purpose       :
// // Special Notes :
// // Scope         : public
// // Creator       : 
...
//   ArtificialParameterMap::const_iterator artificial_param_it = artificial_parameter_map.find(name);
//   if (artificial_param_it != artificial_parameter_map.end())
//   {
//     value = (*artificial_param_it).second->getValue(device_manager);
// //    throw std::runtime_error("getParameter for ArtificialParameter");
//     found = true;
//   }

//   if (!found)
//   {
//     GlobalParameterMap::const_iterator global_param_it = global_parameter_map.find(name);
...
//   if (!found)
//   {
//     const DeviceEntity *device_entity = device_manager.getDeviceEntity(name);

//     if (device_entity)
//     {
//       std::string paramName = Util::paramNameFromFullParamName(name);
//       found = device_entity->getParam(paramName, value);
//       if (paramName == "")
//         found = true;
//     }
//   }
...
//   if (!found) {
//     const InstanceVector &mutual_inductor_devices = device_manager.getDevices(MutIndLin::Traits::modelType());

//     for (InstanceVector::const_iterator it = mutual_inductor_devices.begin(); it != mutual_inductor_devices.end(); ++it)
//     {
//       MutIndLin::Instance &mutual_inductor_device = static_cast<MutIndLin::Instance &>(*(*it));

//       std::cout << mutual_inductor_device.getName();
//     }
//   }

//   if (!found)
//     found = measure_manager.getMeasureValue(name, value);

//   return found;
// }

//-----------------------------------------------------------------------------
// Function      : DeviceMgr::setParameter
// Purpose       :
// Special Notes :
// Scope         : public
// Creator       : 
