...
//
// Revision Date  : $Date: 2015/05/13 22:28:57 $
//
   ...
   isOwned_(true)
{
  if (map.numGlobalEntities() < 0)
    ...
    "MultiVector::MultiVector - vector length too short. Vectors must be > 0 in length.");
...
//-----------------------------------------------------------------------------
MultiVector::MultiVector( Epetra_MultiVector * overlapMV, Epetra_Map& parMap, bool isOwned )
...
MultiVector & MultiVector::operator=( const MultiVector & right )
{
  if (this != &right)
  {
    ...
    {
      if (VERBOSE_LINEAR)
        N_ERH_ErrorMgr::report( N_ERH_ErrorMgr::DEV_FATAL_0, "MultiVector being assigned with different Mapping\n");
    }
  }
...
// Creation Date : 05/22/00
//-----------------------------------------------------------------------------
double MultiVector::dotProduct( const MultiVector & y ) const
{
  double result = 0.0;
...
//-----------------------------------------------------------------------------
// Function      : MultiVector::linearCombo
// Purpose       : Linear combination of three MultiVectors:
//                 this = a*x + b*y + c*z
// Special Notes :
// Scope         : Public
// Creator       : Scott A. Hutchinson, SNL, Computational Sciences
...
void MultiVector::linearCombo(const double a, const MultiVector &x,
                                    const double b, const MultiVector &y,
                                    const double c, const MultiVector &z)
{
...
// Creation Date : 05/23/00
//-----------------------------------------------------------------------------
void MultiVector::GEMM(const bool transA, const bool transB,
                             ...
                             const MultiVector &B)
{
  ...
  int PetraError = aMultiVector_->Multiply(tA, tB, alpha, *(A.aMultiVector_), *(B.aMultiVector_), beta);

  if (DEBUG_LINEAR)
  {
    static const char *methodMsg = "MultiVector::GEMM - ";
...
//-----------------------------------------------------------------------------
int MultiVector::wMaxNorm(MultiVector & weights,
                                MultiVector & tmpVector,
                                double * result) const
{
...
// Creation Date : 05/23/00
//-----------------------------------------------------------------------------
void MultiVector::reduce()
{
  N_ERH_ErrorMgr::report( N_ERH_ErrorMgr::DEV_FATAL,
    "MultiVector::reduce Not available in current PETRA!\n" );
}
...
{
  RCP<const Vector> vec = rcp(new Vector((*oMultiVector_)(index),false), true);
  return vec;
}
...
{
  RCP<Vector> vec = rcp(new Vector((*oMultiVector_)(index),false), true);
  return vec;
}
...
//-----------------------------------------------------------------------------
bool MultiVector::vectorImport(MultiVector * vec,
...
//-----------------------------------------------------------------------------
// Function      : MultiVector::vectorExport
...
// Creation Date : 02/09/01
//-----------------------------------------------------------------------------
bool MultiVector::vectorExport(MultiVector * vec,
                                     Epetra_Import * importer)
{
...
//-----------------------------------------------------------------------------
// Function      : MultiVector::exportContribution
// Purpose       :
// Special Notes :
// Scope         : Public
// Creator       : Robert J Hoekstra, SNL, Parallel Computational Sciences
// Creation Date : 06/06/02
//-----------------------------------------------------------------------------
bool MultiVector::exportContribution( bool do_sum )
{
  if( exporter_ )
    if( do_sum )
...
//-----------------------------------------------------------------------------
// Function      : MultiVector::readFromFile
...
// Creation Date : 08/29/03
//-----------------------------------------------------------------------------
void MultiVector::readFromFile( const char * filename )
{
}
...
//-----------------------------------------------------------------------------
// Function      : MultiVector::packedByteCount
// Purpose       :
// Special Notes :
// Scope         : Public
// Creator       : Robert J. Hoekstra, SNL, Parallel Computational Sciences
...
template<>
int
Pack<Linear::MultiVector>::packedByteCount(
  const Linear::MultiVector &   multi_vector)
{
  int count = 0;
  count += sizeof(int); //# vectors
  count += sizeof(int); //local size
  count += multi_vector.numVectors() * multi_vector.aMultiVector_->MyLength() * sizeof(double); //local vectors

  count += sizeof(int); //extern map size
  count += multi_vector.externVectorMap_.size() * (sizeof(int)+sizeof(double));

  return count;
}

//-----------------------------------------------------------------------------
// Function      : MultiVector::pack
// Purpose       :
// Special Notes :
// Scope         : Public
// Creator       : Robert J. Hoekstra, SNL, Parallel Computational Sciences
...
Pack<Linear::MultiVector>::pack(
  ...
  N_PDS_Comm *                  comm )
{
  int nVs = multi_vector.numVectors();
  comm->pack( &nVs, 1, buf, bsize, pos );
  int locLth = multi_vector.aMultiVector_->MyLength();
  comm->pack( &locLth, 1, buf, bsize, pos );

  double * A;
  int stride;
  multi_vector.aMultiVector_->ExtractView( &A, &stride );

  comm->pack( A, locLth * nVs, buf, bsize, pos );

  int size = multi_vector.externVectorMap_.size();
  comm->pack( &size, 1, buf, bsize, pos );

  int loc;
  double val;
  std::map<int,double>::const_iterator iterXM = multi_vector.externVectorMap_.begin();
  std::map<int,double>::const_iterator endXM = multi_vector.externVectorMap_.end ();
...
template<>
void
Pack<Linear::MultiVector>::unpack(
  Linear::MultiVector & multi_vector,
  char *                buf,
  int                   bsize,
  int &                 pos,
  N_PDS_Comm *          comm )
{
  ...
  if( (numV != multi_vector.numVectors()) || (locL != multi_vector.aMultiVector_->MyLength()) )
    N_ERH_ErrorMgr::report( N_ERH_ErrorMgr::DEV_FATAL,
  ...
  for( int i = 0; i < numV; ++i )
    for( int j = 0; j < locL; ++j )
      (*multi_vector.aMultiVector_)[i][j] = tmpVec[loc++];

  int size;
  double val;
  comm->unpack( buf, bsize, pos, &size, 1 );
  multi_vector.externVectorMap_.clear();
  for( int i = 0; i < size; ++i )
  {
    comm->unpack( buf, bsize, pos, &loc, 1 );
    comm->unpack( buf, bsize, pos, &val, 1 );
    multi_vector.externVectorMap_[loc] = val;
  }
}
