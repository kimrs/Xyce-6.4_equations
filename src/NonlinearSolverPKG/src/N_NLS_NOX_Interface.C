...
// ---------------------
//
// Revision Number: $Revision: 1.199 $
//
// Revision Date  : $Date: 2015/09/14 18:54:20 $
//
...
#include <N_LAS_Matrix.h>/* EDIT KIM */
...
bool Interface::initializeAll()
{
  ...
  bool testsok = false;
  if (NLS_MASKED_WRMS_NORMS)
  {
    testsok =
      dcParams_.createStatusTests(pdsMgrPtr_->getPDSComm()->comm(), currSolVectorPtrPtr_, getLoader(), lasSysPtr_->getDeviceMaskVector()) &&
      transientParams_.createStatusTests(pdsMgrPtr_->getPDSComm()->comm(), currSolVectorPtrPtr_, getLoader(), lasSysPtr_->getDeviceMaskVector()) &&
      hbParams_.createStatusTests(pdsMgrPtr_->getPDSComm()->comm(), currSolVectorPtrPtr_, getLoader(), lasSysPtr_->getDeviceMaskVector());
      nlpParams_.createStatusTests(pdsMgrPtr_->getPDSComm()->comm(), currSolVectorPtrPtr_, getLoader(), lasSysPtr_->getDeviceMaskVector());
  }
  else
  {
    testsok =
      dcParams_.createStatusTests(pdsMgrPtr_->getPDSComm()->comm(), currSolVectorPtrPtr_, getLoader() ) &&
      transientParams_.createStatusTests(pdsMgrPtr_->getPDSComm()->comm(), currSolVectorPtrPtr_, getLoader()) &&
      hbParams_.createStatusTests(pdsMgrPtr_->getPDSComm()->comm(), currSolVectorPtrPtr_, getLoader());
      nlpParams_.createStatusTests(pdsMgrPtr_->getPDSComm()->comm(), currSolVectorPtrPtr_, getLoader());
  }
...
int Interface::spiceStrategy ( ParameterSet* paramsPtr )
{
  ...
  int isuccess = stdNewtonSolve (paramsPtr); /* NOTE for the bsim6, this fails */
  /* Does appear to change the matrices but resets them before trying again */
  ...
  if (isuccess < 0) // attempt gmin stepping.
  {
    ...
    Vector tmpVec(**nextSolVectorPtrPtr_, *lasSysPtr_);
  ...
  /* EDIT KIM */
    /* printing out the first Jacobian and rhs*/
//    std::ofstream ofs;
//    ofs.open("first_rhs.txt");
//    rhsVectorPtr_->printPetraObject(ofs);
//    ofs.close();
//    ofs.open("first_jacobian.txt");
//    jacobianMatrixPtr_->printPetraObject(ofs);
//    ofs.close();
  /* END KIM */

    isuccess=gminSteppingSolve ( paramsPtr ); /* NOTE for the bsim6, this succeds */
    /* printing out the Jacobian and rhs modified by gminSteppingSolve*/
//    ofs.open("post_rhs.txt");
//    rhsVectorPtr_->printPetraObject(ofs);
//    ofs.close();
//    ofs.open("post_jacobian.txt");
//    jacobianMatrixPtr_->printPetraObject(ofs);
//    ofs.close();
...
int Interface::stdNewtonSolve ( ParameterSet* paramsPtr )
{
  ...
  NOX::StatusTest::StatusType status = solverPtr_->solve();
  /* EDIT KIM */
  std::ofstream ofs("first_soln.txt");
  solverPtr_->getSolutionGroup().getX().print(ofs); /* X not changed */
  ofs.close();
  /* END KIM */
...
int Interface::naturalParameterContinuationSolve ( ParameterSet* paramsPtr )
{
  ...
  if (!skipDFDP)
  {
    dsPtr_->allocateSensitivityArrays(lasSysPtr_->builder(), numParam);
  }
...
int Interface::mosfetContinuationSolve ( ParameterSet* paramsPtr )
{
  ...
  if ( numParam > 1 ) {
    Report::UserWarning() << "Using \"continuation=2\" currently does not support vectorized loca parameters.  Only the first values in each list will be used";
  }
...
int Interface::gminSteppingSolve ( ParameterSet* paramsPtr )
{
  ...
  /* NOTE KIM: should probably see if the matrices are available here */
 // sharedSystemPtr_->getJacobian().printPetraObject(std::cout);
  LOCA::Abstract::Iterator::IteratorStatus locaStatus = stepperPtr_->run();
  /* NOTE KIM: should probably see if the matrices are available here */
//  sharedSystemPtr_->getJacobian().printPetraObject(std::cout);
...
{
    /* EDIT KIM */
    (*nextSolVectorPtrPtr_)->printPetraObject(std::cout); /* Soln has changed here */
    std::cout << "done\n";
...
//-----------------------------------------------------------------------------
// Function      : Interface::copySolnVectors()
//
...
{
  Nonlinear::NonLinearSolver::setX0_();
  return true;
}
   ...
   bool isValidF)
{
  ...
  if (!skipDFDP)
  {
    ...
    loadSensitivityResiduals (difference, forceFD, sqrtEta, netlistFilename_, 
    ...
    std::vector<Linear::Vector *> & sensRHSPtrVector = dsPtr_->sensRHSPtrVector;
    for (int i = 0; i < locaPVec.length(); ++i) 
    {
      ...
      Linear::Vector & tmp = *(sensRHSPtrVector[index]);
