...
void NoTimeIntegration::obtainResidual ()
{
  /* EDIT KIM */
  std::ofstream ofs;
  ofs.open("compF_rhs_0.txt");
  ds.RHSVectorPtr->printPetraObject(ofs);
  ofs.close();
  ofs.open("compF_F.txt");
  ds.daeFVectorPtr->printPetraObject(ofs);
  ofs.close();
  ofs.open("compF_B.txt");
  ds.daeBVectorPtr->printPetraObject(ofs);
  ofs.close();
  ofs.open("compF_dFdxdVp.txt");
  ds.dFdxdVpVectorPtr->printPetraObject(ofs);
  ofs.close();
  /* END KIM */

  ds.RHSVectorPtr->linearCombo(0.0,*ds.RHSVectorPtr,+1.0,*ds.daeFVectorPtr);
  ...
  if (ds.limiterFlag)
  {
    (ds.RHSVectorPtr)->daxpy(*(ds.RHSVectorPtr), +1.0, *(ds.dFdxdVpVectorPtr));
  }

  /* PRINT EDIT KIM */
  ofs.open("compF_rhs_1.txt");
  ds.RHSVectorPtr->printPetraObject(ofs);
  ofs.close();
  /* EDIT KIM */
...
//  std::cout << "dFdx: \n";
//  std::ofstream os;
//  os.open("first_dFdx.txt");
//  dFdx.printPetraObject(std::cout);
//  os.close();
//
//  os.open("first_dQdx.txt");
//  std::cout << "dQdx: \n";
  ...
  bool                          skipPrintLineOutput )
{
  ...
  outputManagerAdapter.tranOutput( time, *solnVecPtr, 
                                   *ds.currStatePtr, *ds.currStorePtr, *ds.tmpLeadCurrentVectorPtr, *ds.tmpLeadDeltaVPtr, *ds.tmpLeadCurrentQDerivVectorPtr,
                                   ds.objectiveVec_, 
