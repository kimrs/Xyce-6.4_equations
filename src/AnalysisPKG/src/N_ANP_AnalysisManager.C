...
// ---------------------
// Revision Number: $Revision: 1.265 $
...
analysisModeName(
  Mode          mode) 
{
  static const char * const mode_names[] = {"Invalid", "DC OP", "DC Sweep", "Transient", "MPDE", "HB", "AC", "NOISE", "MOR"};
...
//-----------------------------------------------------------------------------
///
/// Constructs the analysis manager.  The analysis manager is responsible for the construction and control of the top
/// level analysis.
///
/// @invariant
///
...
void
AnalysisManager::createTimeIntegratorMethod(
...
///
/// @invariant There be gremlins here.  The datas tore, step error control, time integrator (managed by the working
  ...
  Device::DeviceMgr &           device_manager)
{
  ...
  int solutionSize = linear_system.getSolutionSize();
  int stateSize    = linear_system.getStateSize();
  ...
  dataStore_ = new TimeIntg::DataStore(solutionSize, stateSize, tia_params.maxOrder, linear_system, linear_system.builder());
  ...
  // register current:
  linear_system.registerCurrStaVector(&dataStore_->currStatePtr);
  linear_system.registerCurrSolVector(&dataStore_->currSolutionPtr);

  // register next:
  linear_system.registerNextStaVector(&dataStore_->nextStatePtr);
  linear_system.registerNextSolVector(&dataStore_->nextSolutionPtr);

  // register last:
  linear_system.registerLastStaVector(&dataStore_->lastStatePtr);
  linear_system.registerLastSolVector(&dataStore_->lastSolutionPtr);

  linear_system.registerFlagSolVector(&dataStore_->flagSolutionPtr);

  // register temporaries:
  linear_system.registerTmpSolVector(&dataStore_->tmpSolVectorPtr);
  linear_system.registerTmpStaVector(&dataStore_->tmpStaVectorPtr);
  linear_system.registerTmpStaDerivVector(&dataStore_->tmpStaDerivPtr);
  linear_system.registerTmpStaDivDiffVector(&dataStore_->tmpStaDivDiffPtr);

  // register next derivatives:
  linear_system.registerNextSolDerivVector(&dataStore_->nextSolutionDerivPtr);
  linear_system.registerNextStaDerivVector(&dataStore_->nextStateDerivPtr);
  ...
  linear_system.registerDAEQVector(dataStore_->daeQVectorPtr);
  linear_system.registerDAEFVector(dataStore_->daeFVectorPtr);
  linear_system.registerDAEBVector(dataStore_->daeBVectorPtr);

  /* EDIT KIM */
  dataStore_->daeFVectorPtr->printPetraObject(std::cout);
  device_manager.F_Strings.init_streams(&dataStore_->daeFVectorPtr->epetraObj(), "F");
  device_manager.Q_Strings.init_streams(&dataStore_->daeQVectorPtr->epetraObj(), "Q");
  device_manager.B_Strings.init_streams(&dataStore_->daeBVectorPtr->epetraObj(), "B");
  /* NED KIM */

  // DAE formulation matrices
  linear_system.registerDAEdQdxMatrix(dataStore_->dQdxMatrixPtr);
  linear_system.registerDAEdFdxMatrix(dataStore_->dFdxMatrixPtr);
...
///
bool
AnalysisManager::getBlockAnalysisFlag() const
{
...
///
void
AnalysisManager::allocateAnalysisObject(
  AnalysisCreatorRegistry &     analysis_registry)
{
  // .OP, but no analysis creates a dcsweep analysis.
  if (dotOpSpecified_ && (analysisCreatorVector_.empty() || (analysisCreatorVector_.size() == 1 && analysisCreatorVector_.front()->isType<Step>())))
  {
    for (CreatorVector::const_iterator it = analysis_registry.begin(), end = analysis_registry.end(); it != end; ++it) 
    {
  ...
  for (CreatorVector::const_iterator it = analysisCreatorVector_.begin(), end = analysisCreatorVector_.end(); it != end; ++it) 
  {
    if (!(*it)->isType<Step>()) 
    {
      primaryAnalysisObject_ = (*it)->create();
  ...
  for (CreatorSet::const_iterator it = processorCreatorSet_.begin(), end = processorCreatorSet_.end(); it != end; ++it)
  {
...
{
  dataStore_->computeDividedDifferences();
...
{
  dataStore_->computeDivDiffsBlock(solGIDList, staGIDList);
...
//-----------------------------------------------------------------------------
bool
AnalysisManager::restoreRestartData(
  char *                        buf,
  int                           bsize,
  int &                         pos,
  Parallel::Communicator *      comm,
  bool                          pack)
{
  return stepErrorControl_->restoreRestartData(buf, bsize, pos, comm, pack, getTIAParams().initialTime);
}
...
{
  return dataStore_->getSolnVarData( gid, varData );
}
...
//-----------------------------------------------------------------------------
bool
AnalysisManager::getStateVarData(
  const int &           gid,
  std::vector<double> & varData ) const
{
  return dataStore_->getStateVarData( gid, varData );
}
...
bool AnalysisManager::getStoreVarData( const int & gid,
                                              std::vector<double> & varData ) const
{
  return dataStore_->getStoreVarData( gid, varData );
}
//-----------------------------------------------------------------------------
...
{
  return dataStore_->setSolnVarData( gid, varData );
}
...
bool AnalysisManager::setStateVarData( const int & gid,
                                              const std::vector<double> & varData )
{
  return dataStore_->setStateVarData( gid, varData );
}
...
bool AnalysisManager::setStoreVarData( const int & gid,
                                              const std::vector<double> & varData )
{
  return dataStore_->setStoreVarData( gid, varData );
}


const TimeIntg::TIAParams &
AnalysisManager::getTIAParams() const
{
  return primaryAnalysisObject_->getTIAParams();
}

TimeIntg::TIAParams &
AnalysisManager::getTIAParams()
{
  return primaryAnalysisObject_->getTIAParams();
}
...
AnalysisManager::setPauseTime(
  ...
  double        initial_time)
{
  stepErrorControl_->setBreakPoint(Util::BreakPoint(pause_time, Util::BreakPoint::PAUSE), initial_time);
}

DCOPType
AnalysisManager::getDCOPSolve() const
{
...
{
  options_manager.addCommandProcessor("OP", IO::createRegistrationOptions(analysis_manager, &AnalysisManager::setOPAnalysisParams));
  options_manager.addCommandProcessor("SENS", IO::createRegistrationOptions(analysis_manager, &AnalysisManager::setSensOptions));
